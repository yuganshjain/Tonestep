...
private enum WidgetKey {
    ...
    static let suiteName = "group.com.yugansh.EarIQ"
}

// MARK: - Timeline entry

struct EarIQEntry: TimelineEntry {
    let date: Date
    let streak: Int
...
struct EarIQProvider: TimelineProvider {
    func placeholder(in context: Context) -> EarIQEntry {
        EarIQEntry(date: Date(), streak: 7, levelName: "Chord Finder", xp: 950, levelProgress: 0.6, todayDone: false)
    }

    func getSnapshot(in context: Context, completion: @escaping (EarIQEntry) -> Void) {
        completion(currentEntry())
    }

    func getTimeline(in context: Context, completion: @escaping (Timeline<EarIQEntry>) -> Void) {
    ...
    private func currentEntry() -> EarIQEntry {
...
struct SmallWidgetView: View {
    ...
    var body: some View {
        ...
        .widgetURL(URL(string: "eariq://today"))
    }
}
            ...
            HStack(spacing: 0) {
                ...
                VStack(alignment: .leading, spacing: 8) {
                    HStack(spacing: 4) {
                        ...
                        Text("EarIQ").font(.system(size: 12, weight: .bold)).foregroundStyle(.white.opacity(0.7))
                    }
...
struct EarIQWidget: Widget {
    let kind = "EarIQWidget"

    var body: some WidgetConfiguration {
        StaticConfiguration(kind: kind, provider: EarIQProvider()) { entry in
        ...
        .configurationDisplayName("EarIQ Streak")
...
@main
struct EarIQWidgetBundle: WidgetBundle {
    var body: some Widget { EarIQWidget() }
}
